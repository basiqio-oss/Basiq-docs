...
<Grid columns="3" gap="20px" style={{ gridTemplateColumns: 'repeat(auto‑fit, minmax(200px, 1fr))' }}>
  ...
  <CoolCard header="API Reference" href="https://api.basiq.io/reference/introduction" icon="fa-duotone fa-solid fa-code-simple" text="Full OpenAPI specification & endpoints for Connect, Enrich, CDR Insights, Reporting, Identity, Platform, Webhooks" />
...
<Grid columns="3" gapX="30px" gapY="30px" paddingY="1.5em" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))' }}>
  <CoolTile header={<span>Create Insight <span style={{ color: '#f59e0b', fontSize: '0.8em' }}>NEW</span></span>} href="https://api.basiq.io/reference/createinsight" icon="fa-duotone fa-solid fa-plus-circle" text="Create an insight for one or more users in a single request. Supports all five verification types: Account, Balance, Identity, Income, and Expense Ratio." />

  <CoolTile header={<span>List Insight Types <span style={{ color: '#f59e0b', fontSize: '0.8em' }}>NEW</span></span>} href="https://api.basiq.io/reference/getinsighttypes" icon="fa-duotone fa-solid fa-list" text="Returns all supported Insight types and the input schema each type expects. Designed as static reference data for discoverability and validation." />

  <CoolTile header={<span>Retrieve Insight Type <span style={{ color: '#f59e0b', fontSize: '0.8em' }}>NEW</span></span>} href="https://api.basiq.io/reference/getinsighttype" icon="fa-duotone fa-solid fa-magnifying-glass" text="Returns the definition of a single Insight type by ID, including its expected input schema." />
  ...
  <CoolTile header="Account Balance Verification" href="https://api.basiq.io/reference/getbalanceinsights" icon="fa-duotone fa-solid fa-scale-balanced" text="Query account balance insights with flexible filtering (min/max parameters). The verification result is now returned under balance instead of accountNumber." />
