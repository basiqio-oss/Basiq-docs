---
title: UI Integration Options
excerpt: ''
deprecated: false
hidden: false
metadata:
  title: ''
  description: ''
  robots: noindex
next:
  description: >-
    When you're ready to start building our your UI, use one of our guides
    below.
  pages:
    - type: basic
      slug: basiqs-simple-ui
      title: Basiq's simple UI
    - type: basic
      slug: build-your-own-1
      title: Build your own
---
If you've just started making calls to the Basiq API, it is a good time to consider your UI integration options. If you haven't, check out our [quickstart](https://api.basiq.io/docs/getting-started-1) to get up and running. 
[block:image]
{
  "images": [
    {
      "image": [
        "https://files.readme.io/e4326a0-Group_6_1.svg",
        "Group 6 (1).svg",
        1065,
        374,
        "#4f6772"
      ],
      "sizing": "80"
    }
  ]
}
[/block]
Basiq provides a number of options to ensure your end user has a seamless experience when connecting their financial institutions to your application.
[block:api-header]
{
  "title": "Basiq's simple UI: For your MVP"
}
[/block]
Embedding the BasiqConnect Bank Connector is the fastest way to get your application’s UI up and running. For this reason, most of our partners use this for their MVP, proof of concept, or hi fidelity prototypes, and will eventually build their own as their application matures. 

This`npm` package gives you a plug and play component to speed up the dev process and get you to market faster. 
[block:api-header]
{
  "title": "Build your own: For a native experience"
}
[/block]
This option is more often used when our customers are moving away from their minimum viable product/proof of concept, and looking to create a native UI component for their application. 

Our step by step guide on how to use the `/institutions` endpoint will guide you to creating a native, seamless, and UX centric component for your application.